...
9. 部署 Supabase Edge Function（可选）
   ...
   本项目提供了以下三个云端函数（Edge Function），用于隐藏 API Key、避免浏览器跨域请求限制和定时同步数据：
   ...
   **配置步骤：**
   ...
   - **部署 `analyze-fund` 函数（v2版本新增）：**
     ...
     - 输入函数名称 `analyze-fund`和复制 `supabase/functions/analyze-fund/index.ts` 内容到编辑器中，点击 **Create**
   ...
   - **部署 `fund-valuation-ranking` 函数：**
     ...
     - 输入函数名称 `fund-valuation-ranking`和复制 `supabase/functions/fund-valuation-ranking/index.ts` 内容到编辑器中，点击 **Create**
     - 到该函数的 Settings 页，取消 **Verify JWT with legacy secret** 选项
   - **部署 `sector-fund-flow-sync` 函数：**
     - 该函数用于定时同步板块资金流向数据，**无需用户登录**，使用 `service_role` 密钥绕过 RLS
     - 推荐使用 Supabase CLI 部署（函数使用 Deno 第三方模块，Editor 在线编辑不支持）：
       ```bash
       supabase functions deploy sector-fund-flow-sync --no-verify-jwt
       ```
     - 部署后需创建定时调度任务（在 Supabase SQL Editor 执行，分早盘和午盘两个任务）：
       ```sql
       ...
       ```
     - 函数特性：
       - 仅在 A 股交易时间段运行（周一至周五 9:30-11:30、13:00-15:00），非交易时段自动跳过
       - 仅同步 `fund_secid` 表中已关联基金（`related_sector` 非空）的板块
       - 每 5 分钟拉取一次板块资金流向和涨跌幅，写入 `fund_topic` 表
     - 验证：可在 Edge Functions 页面手动触发测试
